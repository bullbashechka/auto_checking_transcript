...
## Шаг 6. Запустить Telegram-бота и узнать свой ID
...
```
INFO root: Starting bot. Whitelist: (пусто — открыто всем)
```

Пока `ALLOWED_USER_IDS` пуст — доступ открыт всем. Это нужно только для одного раза, чтобы узнать свой ID:

1. В Telegram найди своего бота по username (тот, что задал в шаге 2).
2. Нажми **Start** или напиши `/start`.
3. Напиши `/id` — бот ответит твоим Telegram ID, например:
   ```
   Твой Telegram ID: 123456789
   ```
4. Останови бота в терминале: `Ctrl+C`.

---

## Шаг 7. Закрыть бота whitelist'ом

Открой `.env` и впиши свой ID (и ID коллег, если их уже знаешь — через запятую):
...
1. Попроси коллегу написать боту `/id`. **Но это работает только если он уже в whitelist!**
   - Альтернатива: пусть напишет [@userinfobot](https://t.me/userinfobot) — это сторонний бот, который покажет свой Telegram ID.
